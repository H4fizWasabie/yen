...
| Agent/tool loop | `packages/agent/src/agent-loop.ts:155-371`, `packages/coding-agent/src/core/agent-session.ts:2177-2308`, `packages/coding-agent/src/core/session-manager.ts:486-527` | event-order, settled-lifecycle, tool, error, abort, steering-priority, usage-persistence, provider-backed compaction, opt-in pre-prompt threshold, atomic failure, and live provider tests | partial; overflow retry, full event payload, and compaction retry/settings parity open |
...
| Telegram | `packages/telegram/src/index.ts:183-285`, `:340-385`, `:630-700` | dedicated unit, Unicode chunking test, valid token, live reply, owner guard, `/stop` unit test | partial; rich rendering, reply threading, images, and live long-reply acceptance deferred |

...
| Agent/tool loop | `packages/agent/src/agent-loop.ts:155-371`, `packages/coding-agent/src/core/agent-session.ts:662-756`, `packages/coding-agent/src/core/session-manager.ts:486-527` | event-order, settled-lifecycle, tool, error, abort, steering-priority, usage-persistence, compaction-boundary, and live provider tests | partial; automatic summarization, full event payload, and compaction retry parity open |

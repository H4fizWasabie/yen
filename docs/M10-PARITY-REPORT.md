...
memory, restart, backup, and rollback checks, but live `/stop` and concurrent
queued-turn traces are not yet recorded and full TypeScript parity is not
claimed.
...
| Telegram | `packages/telegram/src/index.ts:340-385`, `:630-700` | dedicated unit, valid token, live reply, owner guard, `/stop` unit test | partial; live `/stop` open |
...
| Operations | deployed TypeScript systemd units | Go systemd units, health, journald, verified backup, rollback and restore | partial; fresh-rollout script open |

## Remaining cutover gates

- Capture live Telegram `/stop` response and durable canceled turn.
- Capture two concurrent same-conversation submissions and FIFO settlement.

...
Built-in tool edge cases now close the remaining four narrow boundaries. The
document converter preserves markitdown stderr across post-start failures,
web search accepts extension-registry interception, context discovery exposes
shadow diagnostics while preserving candidate precedence, and explorer runs
through a hydrated read-only tool catalog with strict footer accounting and
extension provider hooks. Oracle authority is
`packages/coding-agent/src/core/tools/convert-doc.ts:18-35`,
`packages/coding-agent/src/core/tools/web-search.ts:17-71`,
`packages/coding-agent/src/core/resource-loader.ts:87-105,211-258`, and the
later-added `packages/coding-agent/src/core/explorer.ts:36-38,117-123,246-328`
(the pinned hash named by Goal 6 does not contain `explorer.ts`). Go evidence
is the four tool implementations and their focused tests, plus the runtime
registry wiring. The full test, race, vet, build, and diff gates pass.
...
| Built-in document conversion | `packages/coding-agent/src/core/tools/convert-doc.ts:18-35`, `packages/coding-agent/test/convert-doc.test.ts` | `internal/codingagent/convert_doc.go` resolves the workspace path, invokes `markitdown`, trims output, preserves the empty-document response, enforces the oracle's 2,000,000-byte stdout ceiling, and preserves non-empty stderr for command, read, and output-limit failures; `convert_doc_test.go` covers the boundaries | accepted for the documented convert-doc contract |
| Built-in web search | `packages/coding-agent/src/core/tools/web-search.ts:17-71` | `internal/codingagent/web_search.go` posts Tavily's `{query,max_results:5}` request, tries filtered keys in order, formats answer/results, snapshots the Yen key list at construction, and routes calls/results through the real extension registry; `web_search_test.go` covers fallback, environment snapshots, and replacement interception | accepted for the current Tavily and registry interception contract |
| Context resource discovery | `packages/coding-agent/src/core/resource-loader.ts:87-105,211-258` | `internal/codingagent/prompt.go` preserves per-directory candidate precedence, global persona/ancestor ordering, linked-worktree shadowing, and exposes `ContextMessageWithDiagnostics` for shadowed lower-priority files; `prompt_test.go` covers empty shadowing, directories, ordering, and diagnostics | accepted for the context-file contract; extension-contributed skills/prompts/themes remain on the separate extension-resource path |
...
| Explorer turn budget | `packages/coding-agent/src/core/explorer.ts:36-38`, `:117-123`, `:246-328` | `internal/codingagent/explore.go` builds the four-tool read-only catalog, enforces 8 quick-scan or 15 deep-map provider calls plus 200K/400K input-token ceilings, accounts for a strict `~<K> in, <turns>/<max> turns` footer, and passes registry hooks into the nested provider loop; `explore_test.go` covers hard stops and malformed-footer repair | accepted for the explorer harness contract; the Goal 6 pinned hash lacks this oracle file, so catalog/model hydration is evidenced against its later oracle addition |

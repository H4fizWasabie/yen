...
| GitHub Copilot headers | `packages/ai/src/providers/github-copilot.ts`, `packages/ai/src/api/github-copilot-headers.ts`, `packages/ai/src/api/openai-completions.ts:730-740`, `packages/ai/src/auth/oauth/github-copilot.ts:22-30,208-325,430-460` | `d4ce91b` adds Yen-owned `YEN_COPILOT_GITHUB_TOKEN`, OpenAI-compatible endpoint configuration, `X-Initiator`, `Openai-Intent`, and image-aware `Copilot-Vision-Request`; `430016c` adds the default github.com device OAuth flow and `/login github-copilot`; `internal/auth/copilot_test.go` verifies device polling, token exchange, URL validation, and Copilot headers | partial; enterprise-domain OAuth, refresh/model enablement, catalog filtering, and Responses/Anthropic protocol variants remain open |

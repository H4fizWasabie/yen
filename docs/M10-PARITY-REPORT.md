...
| RPC | `12a739c`, `808ff64`, JSONL server/client, prompt/events/state/messages/abort, branch/artifact/session-name/fork/import/switch/fork-message/stats/set-model/cycle-model/model-catalog/thinking-level/retry controls, Unix socket | complete command matrix and extension UI protocol |
...
The current verified code gate is 210 tests, race tests, vet, and diff checks.
The side-by-side VPS read-back places the tested release at
`/opt/yen/releases/3d1da1e`; both Yen units and both Theoses2 units remain

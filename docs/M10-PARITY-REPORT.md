...
The selector review fix is now closed: Enter confirms the highlighted option,
q/Escape cancel, and j/k or arrow navigation redraws the selected row. Oracle
authority is the selector lifecycle at
`packages/coding-agent/src/modes/interactive/interactive-mode.ts:4281-4305`.
Go evidence is `internal/tui/tui.go` and
`TestSelectJThenEnterReturnsHighlightedOption`.
...
| CLI interactive increment | `packages/coding-agent/src/modes/interactive/interactive-mode.ts:2796-2928,4281-4305,4736-4770,5094-5135` | ANSI scrollback/status/input screen plus scripted `/resume` and `/model` selectors in `internal/tui/tui.go` and `cmd/theoses/main.go`, covered by `TestInteractiveRunRendersScrollbackStatusAndInput`, `TestInteractiveResumeSelectsSessionFromScriptedInput`, `TestInteractiveModelSelectorChangesProviderFromScriptedInput`, and `TestSelectJThenEnterReturnsHighlightedOption`; extension UI components remain open |

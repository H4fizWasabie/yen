...
| CLI | `packages/coding-agent/src/modes/interactive/interactive-mode.ts:2800-2920`, `:5889-5910`, `/name`, `/session`, `/compact`, `/stats`, `/model`, `/thinking`, `/retry`, and `/trust` command behavior | `cmd/theoses/main.go`, shared `session.Stats`, provider control helpers, trust store, and command tests; full TUI rendering, selectors, and remaining slash commands remain open |
...
The current verified code gate is 239 tests, race tests, vet, and diff checks.
The side-by-side VPS read-back places the tested release at
`/opt/yen/releases/45b246e`; both Yen units and both Theoses2 units remain

...
| Telegram | `packages/telegram/src/index.ts:27-33`, `:193-270`, `:290-316`, `:340-385`, `:630-700` | dedicated unit, text/caption fallback for messages and quoted replies, case-insensitive `stop`/`halt`/`/stop`/`/cancel` controls (plus Yen `/abort`), bounded `sendRichMessage` attempt with classic fallback, TypeScript 4,000-character Unicode chunking, reply-context/target tests, cancellable typing-action loop, valid token, live reply, owner guard | partial; rich formatting/tool status, images, and live rich/long-reply/reply acceptance deferred |

...
| Resources and trust | `3b6aa0e`, `d7bc219`, `ca967c4`, bounded context, lazy skills, direct markdown skill discovery, disabled skills remain explicit-only, settings, trust store; steering/follow-up queue modes and auto-compaction now load from settings at startup; in-process extension registry composes hooks and exposes commands/renderers | dynamic extension loading, shortcuts/flags, session actions, and full settings parity |

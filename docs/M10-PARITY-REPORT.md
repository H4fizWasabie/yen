...
| RPC | `12a739c`, `808ff64`, JSONL server/client, prompt/events/state/messages/abort, branch/artifact/session-name/fork/import/switch/fork-message/stats/set-model queries, Unix socket | complete command matrix and extension UI protocol |
...
| Provider configuration | `49794a2`, Yen-owned provider/model/key resolution; `d6c176c` installer key isolation; `197901d`, `ab7677e`, `521b164` Anthropic Messages streaming, stop mapping, and retry; runtime `set_model` for supported protocols | full provider protocols, catalog, OAuth, and auth UI |
...
The current verified code gate is 204 tests, race tests, vet, and diff checks.
The side-by-side VPS read-back places the tested release at
`/opt/yen/releases/a4f4eaa`; both Yen units and both Theoses2 units remain

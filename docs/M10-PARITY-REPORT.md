...
| Agent/tool loop | `packages/agent/src/agent-loop.ts:155-371`, `packages/coding-agent/src/core/agent-session.ts:662-756`, `packages/coding-agent/src/core/session-manager.ts:486-527` | event-order, settled-lifecycle, tool, error, abort, steering-priority, usage-persistence, provider-backed compaction, atomic failure, and live provider tests | partial; automatic threshold scheduling, full event payload, and compaction retry/settings parity open |

...
./internal/provider/...`, `go vet ./...`, `go build ./...`) pass.

**Live-verified 2026-09-17 after release `v0.1.4` deployment.** Deployed to
the pilot VPS and confirmed with an isolated test against real OpenRouter
traffic (a scratch program using Yen's exact production provider wiring —
`providerpkg.NewConfigured("openrouter", "z-ai/glm-5.3-flash")` — with a
~3,300-token stable prefix and four calls separated by real 75-second
gaps, run directly on the VPS and torn down afterward; no production
conversation data was touched). Result: the first call missed as expected
(`cacheRead: 0`), and all three follow-up calls after 75-second gaps hit
(`cacheRead: 3328` each) — compare to the pre-fix live evidence above,
where gaps as short as 42 seconds reliably missed. (A first attempt with
tiny 20-70 token prompts also missed on every call regardless of the
header; that's expected and not part of this result — most providers have
a minimum prompt size below which caching doesn't engage at all,
independent of session affinity, which is why the test was redone with a
realistic prefix size.)
...
`provider.Next` with no token-limit constraint, unlike the oracle's
`distillMemory` (`packages/coding-agent/src/core/compaction/compaction.ts:816-848`),
which caps the call's output tokens to
`Math.min(Math.floor(0.8 * reserveTokens), model.maxTokens > 0 ? model.maxTokens : Infinity)`
before every distillation call — an unbounded best-effort background
extraction could itself run away on output tokens instead of staying a cheap
safety net.

Fixed: a new `providerpkg.SetMaxTokens(provider, maxTokens)`
(`internal/provider/config.go`), following the same generic per-provider
setter pattern as `SetThinkingLevel`/`SetRetryMax`, currently supports
`OpenAICompletions` (the production main-conversation provider path) and
returns an error for providers without output-token control so callers can
proceed uncapped rather than fail. `DistillMemory` gained an explicit
`reserveTokens int` parameter and applies `floor(0.8 * reserveTokens)` via
`SetMaxTokens` best-effort before calling the provider, matching the
oracle's bound exactly; `Runner.distillDroppedMemory`
(`internal/runtime/runtime.go`) now passes `r.AutoCompactReserveTokens`
(defaulting to 16,384 via `AutoCompactReserveTokensFromEnv`, matching the
oracle's `DEFAULT_COMPACTION_SETTINGS.reserveTokens`). Go evidence:
`TestSetMaxTokensShapesOpenAICompletions`,
`TestSetMaxTokensRejectsUnsupportedProvider`,
`TestDistillMemoryBoundsOutputTokensForSupportedProviders` (asserts the
live HTTP request body carries `max_completion_tokens: 13107` for
`reserveTokens: 16384`), and
`TestDistillMemoryLeavesProviderUnboundedWhenReserveTokensIsUnset`
(confirms unchanged behavior when unconfigured). Full repository gates
(`go test ./...` — 564 passed, `go test -race
./internal/memory/... ./internal/provider/... ./internal/runtime/...`,
`go vet ./...`, `go build ./...`) pass. Not yet released or deployed.

Remaining gap, not attempted here: only `OpenAICompletions` supports
output-token control today; `AnthropicMessages`, `GoogleGenerativeAI`,
`OpenAIResponses`, `BedrockConverse`, and `TheosesMessages` have no
max-output-tokens field at all, so distillation silently stays unbounded
for those providers. Extending the bound to the rest of the provider
surface is a larger, separate change.
...
VPS, closing the deferred item tracked in
[GH-219](https://github.com/H4fizWasabie/yen/issues/219). TypeScript
authority: `packages/telegram/src/index.ts:430-470` (photo-to-image-content
conversion) and `:536-542,550-560,575-585` (document artifact storage and
`convert_doc` guidance note).

**Photo path**: a real Telegram photo (a medication box) was sent to the
live bot on 2026-09-17T07:06:04Z. The Go adapter converted it to
`"images":["data:image/jpeg;base64,..."]` content on the user message, and
the model correctly read the label text (product name, dosage, batch,
expiry, registration number) from the image — confirmed in session
`conv-5955b65fd7cbaf4f869df7af7f6c1fb6`.

**Document path**: a real Telegram PDF (a Lalamove delivery receipt) was
sent at 2026-09-17T07:14:26Z and stored via the `"telegram document"`
artifact path. This surfaced two real environment/ops bugs rather than an
application-code gap (full root cause and fix in GH-219/PR #220,
`v0.1.3`):

1. `convert_doc` failed with `Tool error: markitdown is not installed`
   even though `markitdown` was present on the host at
   `/usr/local/bin/markitdown` — the `run-telegram`/`run-dashboard`
   systemd wrapper scripts in `deploy/install-side-by-side.sh` stripped
   `PATH` down to `/usr/bin:/bin`, hiding it.
2. The bash tool failed with a dead-directory error because
   `internal/conversation.Registry.Resolve`/`ResolveShared` froze a
   conversation's `WorkspaceID` at first-link-creation time and never
   refreshed it on redeploy, even though callers always pass the current,
   correct workspace on every call
   (`internal/runtime/runtime.go:895`, `cmd/theoses-telegram/main.go:21`).
   Once the conversation's original release directory was cleaned up by
   release rotation, tool execution broke permanently for that
   conversation, independent of the live process's own health.

Both were fixed in `v0.1.3` (`internal/conversation/conversation.go`:
`Resolve`/`ResolveShared` now refresh and persist a changed `WorkspaceID`;
`deploy/install-side-by-side.sh`: wrapper `PATH` now includes
`/usr/local/bin`), covered by
`TestRegistryResolveRefreshesStaleWorkspaceID` and
`TestRegistryResolveSharedRefreshesStaleWorkspaceID`. Re-verified live at
2026-09-17T07:31:21Z: the same conversation's `convert_doc` call
successfully extracted the full receipt text (order ID, price, pickup/
drop-off addresses) from the same PDF, with no manual data migration —
the existing link's `WorkspaceID` self-healed on the next resolve. Full
repository gates (`go test ./...` — 559 passed, `go test -race
./internal/conversation/...`, `go vet ./...`, `go build ./...`) pass.
...
| Operations | deployed TypeScript systemd units | Go systemd units, health, journald, verified backup, rollback/restore, isolated installer acceptance with temporary root/fake systemctl, repeated real-host side-by-side rollout/read-back, fresh 2026-09-15 backup `/var/backups/yen-20260915T093942Z.tgz` (mode 600), and isolated 2026-09-16 VPS experiment with separate units/paths, `YEN_START=0`, mode-600 secrets, health, backup, restore, and release rollback | accepted for the side-by-side pilot; the 2026-09-16 experiment was not a fresh host, so unprivileged fresh-host production rollout remains untested |

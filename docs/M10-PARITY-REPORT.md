...
| Provider | `packages/ai/src/api/openai-completions.ts:699-717`, `:830-930`, `:362-465`, `:523-551`, `:632-647`, `:1260-1370`, `:1553-1575`, `packages/ai/src/utils/overflow.ts:39-171` | deterministic SSE including explicit-null finish reasons, response-body overflow classification, transient exclusion, reasoning-effort request shape, reasoning-field and ordered `reasoning_details` replay with session persistence, response ID/model and raw finish-reason preservation, finish-reason error mapping, and live `z-ai/glm-5.3-flash` OpenRouter reply using Yen-owned credentials | partial; provider matrix open |
...
| Session read-back/migration | `packages/coding-agent/src/core/session-manager.ts:31-104`, `:284-355`, `:357-370`, `:397-430`, `:986-1022`, `:1431-1451`, `:1708-`, `packages/coding-agent/src/core/agent-session-runtime.ts:357-` | Go fixtures migrate v1/v2 JSONL to v3, skip malformed lines before/after the header, read bounded 4 MiB JSONL entries, preserve image metadata and v2 tree links/extension/message payloads, assign v1 IDs/parents, convert `hookMessage`, preserve session-info names, create durable fork copies, validate/copy imported sessions, project the active parent-linked branch, and read flat v3 compaction entries; 135 Go tests | partial; runtime session switching and malformed/truncated-stream behavior beyond the explicit bound remain open |
...
| RPC | `12a739c`, `808ff64`, JSONL server/client, prompt/events/state/messages/abort, branch/artifact/session-name/fork/import queries, Unix socket | complete command matrix and extension UI protocol |
...
The current verified code gate is 201 tests, race tests, vet, and diff checks.
The side-by-side VPS read-back places the tested release at
`/opt/yen/releases/5b39877`; both Yen units and both Theoses2 units remain

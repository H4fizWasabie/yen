...
| Semantic memory | `packages/coding-agent/src/core/tools/memory.ts:18-59`, `packages/coding-agent/src/core/memory-store.ts:9-19` | conversation-scoped `save_note`/`remember`; live favorite-color and probe read-back; consolidation applies only the pinned closed edge vocabulary, with invalid relations rejected by test | partial; automatic consolidation scheduling and full prompt/retry matrix open |

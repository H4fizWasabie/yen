...
| CLI interactive increment | `packages/coding-agent/src/modes/interactive/interactive-mode.ts:2796-2928,4281-4305,4736-4770,5094-5135,1984-2003` | ANSI scrollback/status/input screen, selectors, registered message renderers, extension presentation state, and tool-progress status in `internal/tui` and `cmd/theoses/main.go`; covered by `TestInteractiveRunRendersScrollbackStatusAndInput`, `TestInteractiveResumeSelectsSessionFromScriptedInput`, `TestInteractiveModelSelectorChangesProviderFromScriptedInput`, `TestInteractiveRunRendersExtensionMessage`, `TestHandleExtensionUIUpdatesScreenPresentation`, and `TestInteractiveRunRendersToolProgress`; native custom component layout remains open |

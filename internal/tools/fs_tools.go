...
var fileMutationLocks sync.Map // canonical path -> *sync.Mutex

func fileMutationLock(path string) *sync.Mutex {
	if lock, ok := fileMutationLocks.Load(path); ok {
		return lock.(*sync.Mutex)
	}
	actual, _ := fileMutationLocks.LoadOrStore(path, &sync.Mutex{})
...
filesLoop:
	for _, rel := range files {
		...
		if glob != "" && !matchFindPattern(glob, rel) {
			continue
		}

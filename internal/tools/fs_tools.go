...
func (t ListTool) Execute(ctx context.Context, args map[string]any) (string, error) {
	...
	for _, entry := range entries {
		if _, err := os.Stat(filepath.Join(path, entry.Name())); err == nil {
...
func (t FindTool) Execute(ctx context.Context, args map[string]any) (string, error) {
	...
	files, err := collectSearchFiles(ctx, root)
	if err != nil {
		return "", err
	}
	ignored := ignoredSearchFiles(ctx, root, files)
	var matches []string
	matchCount := 0
	for _, file := range files {
		if ignored[file.relative] {
			continue
		}
		matched := matchFindPattern(pattern, file.relative)
		if matched {
			matchCount++
...
fileLoop:
	for _, file := range files {
		...
		if glob != "" && !matchFindPattern(glob, file.relative) {
			continue
		}
		...
		for i, line := range lines {
			...
			for j := start; j < end; j++ {
				...
				out = append(out, fmt.Sprintf("%s%s%d%s %s", file.relative, separator, j+1, separator, truncateGrepLine(lines[j])))
			}
			if matchCount >= limit {
				limitReached = true
				break fileLoop
...
type searchFile struct{ absolute, relative string }

func collectSearchFiles(ctx context.Context, root string) ([]searchFile, error) {
	...
	err := filepath.WalkDir(root, func(path string, entry os.DirEntry, err error) error {
		...
		files = append(files, searchFile{absolute: path, relative: filepath.ToSlash(rel)})
		return nil
	})
	return files, err
}

// ignoredSearchFiles batches the Git query so a large search does not spawn
// one subprocess per file. Git failures preserve the non-Git-repository
// behavior by treating every path as visible.
func ignoredSearchFiles(ctx context.Context, root string, files []searchFile) map[string]bool {
	if len(files) == 0 {
		return nil
	}
	...
	for _, file := range files {
		input.WriteString(file.relative)
		input.WriteByte(0)
	}
	...
	for _, path := range bytes.Split(output, []byte{0}) {
		if len(path) > 0 {
			ignored[filepath.ToSlash(string(path))] = true
		}
	}
	return ignored
}
...
func (t EditTool) Execute(ctx context.Context, args map[string]any) (string, error) {
	...
	data, err := os.ReadFile(normalizeReadPath(path, t.cwd))
	if err != nil {
		return "", fmt.Errorf("could not edit file: %s: %w", path, err)
	}
	original := string(data)
	edits, err := editArgs(args)
	if err != nil {
		return "", err
	}
	result := original
	...
	for _, edit := range edits {
		...
		if count != 1 {
			return "", fmt.Errorf("oldText must match a unique region in %s (found %d matches)", path, count)
		}
		idx := strings.Index(original, edit.oldText)
		for _, p := range positions {
			if idx < p[1] && idx+len(edit.oldText) > p[0] {
				return "", fmt.Errorf("edits overlap in %s", path)
			}
		}
	...
	for i, p := range positions {
		...
		for _, candidate := range edits {
			if strings.Index(original, candidate.oldText) == p[0] {
				edit = candidate
				break
			}
		}
		result = result[:p[0]] + edit.newText + result[p[1]:]
		_ = i
	}
	if err := os.WriteFile(normalizeReadPath(path, t.cwd), []byte(result), 0o600); err != nil {
		return "", err
	}
...
func editArgs(args map[string]any) ([]textEdit, error) {
	if raw, ok := args["edits"].([]any); ok {
		...
		out := make([]textEdit, 0, len(raw))
		for _, item := range raw {
			m, ok := item.(map[string]any)
			if !ok {
				return nil, fmt.Errorf("invalid edit")
			}
			old, ok1 := m["oldText"].(string)
			newText, ok2 := m["newText"].(string)
			if !ok1 || !ok2 {
				return nil, fmt.Errorf("invalid edit")
			}
			out = append(out, textEdit{old, newText})
		}
		return out, nil
	}
	old, ok1 := args["oldText"].(string)
	newText, ok2 := args["newText"].(string)
	if !ok1 || !ok2 {
		return nil, fmt.Errorf("edits must contain at least one replacement")
	}
	return []textEdit{{old, newText}}, nil
}
